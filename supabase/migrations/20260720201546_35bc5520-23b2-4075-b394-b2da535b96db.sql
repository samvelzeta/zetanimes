...
BEGIN
  ...
  WHERE r.id IN (
    ...
    WHERE pr.reserve_state = 'available'
      ...
      AND NOT EXISTS (
        SELECT 1 FROM public.pending_anime_reserve already
        WHERE already.reserve_state = 'consumed'
          AND already.consumed_at >= date_trunc('week', now())
...
GRANT EXECUTE ON FUNCTION public.consume_weekly_pending_reserve(integer) TO anon;
GRANT EXECUTE ON FUNCTION public.consume_weekly_pending_reserve(integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.consume_weekly_pending_reserve(integer) TO service_role;
