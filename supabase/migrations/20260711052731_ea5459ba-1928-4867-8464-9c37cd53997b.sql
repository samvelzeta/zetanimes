...
BEGIN
  ...
  -- Sorteos especiales (nombres secretos)
  ...
  -- Pool completo (todas las rarezas) para respetar probabilidades
  ...
  IF _pool = 'frame' THEN
    INSERT INTO _pool_items(slug, name, image_url, rarity) VALUES
      -- Básicos / especiales / raros / míticos (del catálogo estático)
      ('neon-orange',   'Neón Zet',            NULL, 'especial'),
      ('sakura',        'Sakura',              NULL, 'especial'),
      ('art-petals',    'Pétalos de sakura',   NULL, 'especial'),
      ('hex-neon',      'Hexágono neón',       NULL, 'raro'),
      ('shield-fire',   'Escudo llameante',    NULL, 'raro'),
      ('art-flames',    'Aro de fuego azul',   NULL, 'raro'),
      ('art-thorns',    'Espinas malditas',    NULL, 'raro'),
      ('diamond-ice',   'Diamante de hielo',   NULL, 'mitico'),
      ('star-gold',     'Estrella dorada',     NULL, 'mitico'),
      ('art-swords',    'Espadas cruzadas',    NULL, 'mitico'),
      ('art-roses',     'Rosas y espinas',     NULL, 'mitico'),
      -- Legendarios
      ('rainbow',       'Arcoíris',            NULL, 'legendario'),
      ('art-wings',     'Alas celestiales',    NULL, 'legendario'),
      ('art-crown',     'Corona real',         NULL, 'legendario'),
      ('art-dragon',    'Dragón dorado',       NULL, 'legendario'),
      ('art-lotus-red', 'Loto rojo',           NULL, 'legendario'),
  ...
  ELSE
    INSERT INTO _pool_items(slug, name, image_url, rarity) VALUES
      ('aurora',            'Aurora',               NULL, 'basico'),
      ('sakura-g',          'Sakura',               NULL, 'basico'),
      ('cyber',             'Cyber city',           NULL, 'especial'),
      ('sunset',            'Atardecer',            NULL, 'especial'),
      ('noir',              'Noir',                 NULL, 'especial'),
      ('ocean',             'Océano',               NULL, 'raro'),
      ('forest',            'Bosque místico',       NULL, 'raro'),
      ('lotus',             'Loto',                 NULL, 'raro'),
      ('gold',              'Dorado',               NULL, 'mitico'),
      ('higanbana',         'Higanbana (muerte)',   NULL, 'mitico'),
      ('ninjas',            'Ninjas nocturnos',     NULL, 'mitico'),
      ('roses',             'Rosas de sangre',      NULL, 'mitico'),
      ('swamp-far-red',     'Pantano lejano',       NULL, 'mitico'),
      ('swords',            'Espadas caídas',       NULL, 'legendario'),
      ('isekai',            'Isekai',               NULL, 'legendario'),
      ('magic',             'Magia elemental',      NULL, 'legendario'),
      ('z-void',            'Guardián del vacío',   NULL, 'legendario'),
      ('swamp-close-red',   'Loto rojo en el lodo', NULL, 'legendario'),
      ('swamp-close-blue',  'Loto azul lunar',      NULL, 'legendario'),
  ...
  -- Sorteo de rareza (probabilidades reales)
  --   básico    64.0%
  --   especial  22.0%
  --   raro      10.0%
  --   mítico     3.0%
  ...
  -- 1) Item no poseído de la rareza sorteada
  ...
  -- 2) Bajar de rareza (NUNCA subir) hasta encontrar algo no poseído
  IF chosen_slug IS NULL THEN
    ...
    WHILE chosen_slug IS NULL AND idx < array_length(rarity_order, 1) LOOP
      ...
      FROM _pool_items p
      WHERE p.rarity = rarity_order[idx]::public.cosmetic_rarity
        AND NOT EXISTS (SELECT 1 FROM public.user_gacha_inventory i WHERE i.user_id=uid AND i.pool=_pool AND i.slug=p.slug)
      ORDER BY random() LIMIT 1;
    END LOOP;
  END IF;

  -- 3) Ya lo tiene todo → duplicado en la rareza original
  IF chosen_slug IS NULL THEN
    is_duplicate := true;
    SELECT slug, name, image_url INTO chosen_slug, chosen_name, chosen_image
