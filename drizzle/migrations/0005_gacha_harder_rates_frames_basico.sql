...
BEGIN
  ...
  IF _pool = 'frame' THEN
    -- Rebalanced: frames now have a real 'basico' tier so the common roll resolves inside the pool
    INSERT INTO _pool_items(slug, name, image_url, rarity) VALUES
      ('neon-orange','Neón Zet',NULL,'basico'),('sakura','Sakura',NULL,'basico'),('art-petals','Pétalos de sakura',NULL,'basico'),
      ('hex-neon','Hexágono neón',NULL,'especial'),('shield-fire','Escudo llameante',NULL,'especial'),('art-flames','Aro de fuego azul',NULL,'especial'),('art-thorns','Espinas malditas',NULL,'especial'),
      ('diamond-ice','Diamante de hielo',NULL,'raro'),('star-gold','Estrella dorada',NULL,'raro'),('art-swords','Espadas cruzadas',NULL,'raro'),('art-roses','Rosas y espinas',NULL,'raro'),
      ('rainbow','Arcoíris',NULL,'mitico'),('art-wings','Alas celestiales',NULL,'mitico'),('art-crown','Corona real',NULL,'mitico'),('art-lotus-red','Loto rojo',NULL,'mitico'),('art-lotus-blue','Loto azul',NULL,'mitico'),
      ('art-dragon','Dragón dorado',NULL,'legendario'),('viking','Casco de Odín',NULL,'legendario'),('twin-blades','Espadas gemelas',NULL,'legendario'),
  ...
  -- HARD PITY: guarantee legendario after 100 pulls
  IF pity_legendary >= 100 THEN
    chosen_rarity := 'legendario'::public.cosmetic_rarity;
    pity_forced := true;
  -- SOFT PITY from pull 85
  ELSIF pity_legendary >= 85 THEN
    r := random();
    IF r < (0.0028 + (pity_legendary - 85) * 0.008) THEN
      chosen_rarity := 'legendario'::public.cosmetic_rarity;
      pity_forced := true;
    END IF;
  END IF;
  ...
  IF NOT pity_forced THEN
    r := random();
    -- Scarcity rates: z 0.02%, legendario 0.28%, mitico 1.2%, raro 4.5%, especial 14%, basico 80%
    chosen_rarity := CASE
      WHEN r < 0.0002 THEN 'z'::public.cosmetic_rarity
      WHEN r < 0.0030 THEN 'legendario'::public.cosmetic_rarity
      WHEN r < 0.0150 THEN 'mitico'::public.cosmetic_rarity
      WHEN r < 0.0600 THEN 'raro'::public.cosmetic_rarity
      WHEN r < 0.2000 THEN 'especial'::public.cosmetic_rarity
  ...
  -- Fallback ONLY downwards (towards basico), never upgrade to a rarer tier
  ...
  -- Everything of that tier (and below) owned: return a duplicate of the LOWEST tier available
  IF chosen_slug IS NULL THEN
    is_duplicate := true;
    SELECT slug, name, image_url, rarity INTO chosen_slug, chosen_name, chosen_image, chosen_rarity
    FROM _pool_items p
    ORDER BY array_position(rarity_order, p.rarity::text) DESC, random()
    LIMIT 1;
  END IF;
