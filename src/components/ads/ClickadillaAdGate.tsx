...
// Lógica:
//   - Alternancia 1 sí / 1 no por episodio (par = anuncio, impar = limpio).
//   - Usa localStorage para persistir entre recargas.
//   - Si pasan más de 30 min sin actividad, el contador se reinicia a 0
//     (el próximo capítulo mostrará anuncio obligatoriamente).
//   - Premium queda exento.
//
// El script Clickadilla se inyecta dentro de #zet-player-container solo cuando
// toca. Se limpia al cambiar de episodio para poder recargarlo en la próxima
// ronda "sí".
...
export default function ClickadillaAdGate({ episodeKey }: Props) {
  ...
  useEffect(() => {
    ...
    // Evita re-procesar el mismo episodio en re-renders.
    ...
    if (
      !lastActivity ||
      storedCounter === null ||
      NOW - parseInt(lastActivity, 10) > INACTIVITY_LIMIT_MS
    ) {
      // Sesión nueva → primer capítulo SIEMPRE con anuncio.
      counter = 0;
    } else if (lastEp === episodeKey) {
      // Mismo episodio (recarga rápida): respetar el valor ya evaluado
