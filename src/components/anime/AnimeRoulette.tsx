  ...
  return (
    <section className="mb-8 px-4">
      ...
      <div className="flex flex-col items-center">
          ...
          <div className="mb-4 flex flex-col items-center animate-[hero-slide-up_0.5s_ease-out_forwards]" style={{ opacity: 0 }}>
            <Link to={`/anime/${result.id}`} className="group text-center">
              <img
                ...
                alt={getTitle(result)}
                className="w-36 h-48 rounded-xl object-cover shadow-lg mx-auto animate-[roulette-pulse_2s_ease-in-out_infinite]"
              />
        ...
        {/* Half-moon arc - bigger, closer */}
        <div className="relative w-full overflow-hidden" style={{ height: `${radius * 0.55 + 40}px` }}>
          <div
            className="absolute left-1/2"
            style={{
              transform: `translateX(-50%) rotate(${rotation}deg)`,
              transition: spinning ? "transform 3.5s cubic-bezier(0.17, 0.67, 0.12, 0.99)" : "none",
              bottom: `-${radius * 0.75}px`,
            }}
          >
            {items.map((anime, i) => {
              ...
              return (
                <div
                  key={anime.id}
                  className="absolute w-[70px] h-[70px] rounded-full overflow-hidden border-2 border-primary/40 shadow-lg"
                  style={{
                    left: `${x}px`,
                    top: `${-y}px`,
